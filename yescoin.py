...
█▄█ █▀█ █▀█ █▄▄ █ █▄█ █ ██▄
    ...
    print(Fore.GREEN + Style.BRIGHT +
          "Update Link: https://github.com/Honguan/yescoin")
...
available_colors = [Fore.GREEN, Fore.YELLOW,
...
def use_special_box(token):
    ...
    try:
        ...
        if result['code'] == 0:
            print(
                f"{Fore.GREEN + Style.BRIGHT}\r[ Chest ] : Activating...", end="", flush=True)
            return True
        else:
            print(
                f"{Fore.RED + Style.BRIGHT}\r[ Chest ] : Failed to activate", end="", flush=True)
            return False
    except Exception as e:
    ...
    try:
        ...
        if result['code'] == 0:
            if result['data']['collectStatus']:
                print(
                    f"{random.choice(available_colors) + Style.BRIGHT}\r[ Chest ] : Collected {result['data']['collectAmount']} Coins                                                     ", flush=True)

                return True, result['data']['collectAmount']
            else:
                print(
                    f"{Fore.RED + Style.BRIGHT}\r[ Chest ] : Tidak ada chest          ", flush=True)
                return True, 0
        else:
            print(
                f"{Fore.RED + Style.BRIGHT}\r[ Chest ] : Failed to collect coins: {result['message']}", end="", flush=True)
            return False, 0
    except Exception as e:
...
def attempt_collect_special_box(token, box_type, initial_coin_count):
    ...
    while coin_count > 0:
        success, collected_amount = collect_from_special_box(
            token, box_type, coin_count)
        if success:
            return collected_amount
        coin_count -= 20  # Kurangi jumlah koin jika gagal karena batas
    print(
        f"{Fore.RED + Style.BRIGHT}\r[ Chest ] : Unable to collect any coins after adjustments", flush=True)
    return 0
...
def recover_coin_pool(token):
    ...
    try:
        ...
        if result['code'] == 0:
            print(
                f"{random.choice(available_colors)+Style.BRIGHT}\r[ Recovery ] : Sukses               ", flush=True)
            return True
        else:
            print(
                f"{Fore.RED + Style.BRIGHT}\r[ Recovery ] : Gagal", flush=True)
            return False
    except Exception as e:
        print(
            f"{Fore.RED + Style.BRIGHT}\r[ Recovery ] : Error: {e}", flush=True)
        return False
...
def fetch_task_list(token):
    ...
    try:
        ...
        if tasks['code'] == 0:
            ...
            print(
                f"{Fore.RED + Style.BRIGHT}\r[ Task ] : Failed to get task list: {tasks['message']}", flush=True)
            return None
    except Exception as e:
...
def finish_task(token, task_id):
    ...
    try:
        ...
        if result['code'] == 0:
            print(
                f"{Fore.GREEN + Style.BRIGHT}\r[ Task ] : Task {task_id} finished. Bonus: {result['data']['bonusAmount']}", flush=True)
            return True
        else:
            # print(result)
            print(
                f"{Fore.RED + Style.BRIGHT}\r[ Task ] : Failed to finish task {task_id}: {result['message']}", flush=True)
            return False
    except Exception as e:
    ...
    if tasks:
        for task in tasks:
            if task['taskStatus'] == 0:  # Jika tugas belum diklaim
                ...
                print(
                    f"{random.choice(available_colors)+Style.BRIGHT}\r[ Task ] : Task already finished          ", flush=True)
...
def upgrade_level(token, current_level, target_level, upgrade_type):
    ...
    while current_level < target_level:
        ...
        if result['code'] == 0:
            current_level += 1
            print(
                f"{random.choice(available_colors)+Style.BRIGHT}\r[ Upgrade ] : {upgrade_type_name} Upgrade to {current_level}            ", flush=True)
        else:
            print(
                f"{random.choice(available_colors)+Style.BRIGHT}\r[ Upgrade ] : Failed to upgrade: {result['message']}        ", flush=True)
            break

    if current_level == target_level:
        print(
            f"{random.choice(available_colors)+Style.BRIGHT}\r[ Upgrade ] : {upgrade_type_name} already at level {current_level}          ", flush=True)
...
def main():
    ...
    while True:
        ...
        for index, token in enumerate(tokens):
            ...
            print(
                f"{random.choice(available_colors)+Style.BRIGHT}\r[ Squad ] : Getting...", end="", flush=True)
            ...
            if squad_info and squad_info['data']['isJoinSquad']:
                ...
                print(
                    f"{random.choice(available_colors)+Style.BRIGHT}\r[ Squad ] : {squad_title} | {squad_members} Members")
            else:
                print(
                    f"{Fore.YELLOW + Style.BRIGHT}\r[ Squad ] : Belum Join Squad. Joining Ghalibie.", end="", flush=True)
                time.sleep(3)
                join_result = join_squad(token, "t.me/ghalibie")
                if join_result:
                    print(
                        f"{random.choice(available_colors) + Style.BRIGHT}\r[ Squad ] : Welcome Pemulung {nickname} - Ghalibie !      ", flush=True)
                else:
                    print(
                        f"{random.choice(available_colors) + Style.BRIGHT}\r[ Squad ] : Failed to join Ghalibie.", flush=True)

            print(
                f"{random.choice(available_colors)+Style.BRIGHT}\r[ Balance ] : Getting...", end="", flush=True)
            balance = fetch_account_info(token)
            if balance is None:
                print(
                    f"{Fore.RED}\r[ Balance ] : Failed to get balance", flush=True)
                continue
            else:
                balance = balance.get('data', {}).get('currentAmount', 0)
                balance = f"{balance:,}".replace(',', '.')
                print(
                    f"{random.choice(available_colors) + Style.BRIGHT}\r[ Balance ] : {balance}           ", flush=True)

            print(
                f"{random.choice(available_colors)+Style.BRIGHT}\r[ Game Info ] : Getting...", end="", flush=True)
            game_info = fetch_account_build_info(token)
            if game_info is None:
                print(
                    f"{Fore.RED}\r[ Game Info ] : Failed to get data", flush=True)
                continue
            else:
                special_box_left_recovery_count = game_info['data'].get(
                    'specialBoxLeftRecoveryCount', 0)
                coin_pool_left_recovery_count = game_info['data'].get(
                ...
                coin_pool_recovery_speed = game_info['data'].get(
                    'coinPoolRecoverySpeed', 0)
                print(
                    f"{random.choice(available_colors)+Style.BRIGHT}\r[ Booster ] : Chest {special_box_left_recovery_count} | Recovery {coin_pool_left_recovery_count}", flush=True)
                print(
                    f"{random.choice(available_colors)+Style.BRIGHT}\r[ Multivalue ] : Level {single_coin_value}", flush=True)
                print(
                    f"{random.choice(available_colors)+Style.BRIGHT}\r[ Coin Limit ] : Level {single_coin_level}", flush=True)
                print(
                    f"{random.choice(available_colors)+Style.BRIGHT}\r[ Fill Rate ] : Level {coin_pool_recovery_speed}", flush=True)
            if cek_task_enable == 'y':
                print(
                    f"{random.choice(available_colors)+Style.BRIGHT}\r[ Task ] : Trying to finish task..", end="", flush=True)
                process_tasks(token)
            time.sleep(2)
            if upgrade_multi_enable == 'y':
                print(
                    f"{random.choice(available_colors)+Style.BRIGHT}\r[ Upgrade ] : Upgrading Multi Value...", end="", flush=True)
                upgrade_level(token, single_coin_value, max_level, '1')
            time.sleep(2)
            if upgrade_fill_enable == 'y':
                print(
                    f"{random.choice(available_colors)+Style.BRIGHT}\r[ Upgrade ] : Upgrading Fill Rate...", end="", flush=True)
            ...
            if collect_info is None:
                ...
                single_coin_value = collect_info['data'].get(
                    'singleCoinValue', 0)
                coin_pool_left_count = collect_info['data'].get(
                    'coinPoolLeftCount', 0)
                print(
                    f"{random.choice(available_colors) + Style.BRIGHT}\r[ Coin Left ] : {coin_pool_left_count}                              ", flush=True)

                print(
                    f"{random.choice(available_colors)+Style.BRIGHT}\r[ Collect ] : Collecting Coin...", end="", flush=True)
                if coin_pool_left_count > 0:
                    ...
                    if collect_result and collect_result.get('code') == 0:
                        ...
                        print(
                            f"{random.choice(available_colors) + Style.BRIGHT}\r[ Collect ] : Collected {random.choice(available_colors)+Style.BRIGHT}{collected_amount}                                                  ", flush=True)
                    else:
                        print(
                            f"{random.choice(available_colors)+Style.BRIGHT}\r[ Collect ] : Failed to collect coins", flush=True)

            print(
                f"{random.choice(available_colors)+Style.BRIGHT}\r[ Chest ] : Trying to activate...", end="", flush=True)
            if game_info and game_info['data'].get('specialBoxLeftRecoveryCount', 0) > 0:
                if use_special_box(token):
                    print(
                        f"{random.choice(available_colors)+Style.BRIGHT}\r[ Chest ] : Collecting...", end="", flush=True)
                    collected_amount = attempt_collect_special_box(
                        token, 2, 240)  # Contoh: box_type=2, coin_count=250
            else:
                print(
                    f"{random.choice(available_colors)+Style.BRIGHT}\r[ Chest ] : No chest available          ", flush=True)
            time.sleep(2)
            print(
                f"{random.choice(available_colors)+Style.BRIGHT}\r[ Recovery ] : Trying to recover...", end="", flush=True)
            game_info = fetch_account_build_info(token)
                ...
                if recover_coin_pool(token):
                    ...
                    if collect_info:
                        coin_pool_left_count = collect_info['data'].get(
                            'coinPoolLeftCount', 0)
                        if coin_pool_left_count > 0:
                            amount = coin_pool_left_count // game_info['data'].get(
                                'singleCoinValue', 1)
                            collect_result = collect_coin(token, amount)
                            if collect_result and collect_result.get('code') == 0:
                                collected_amount = collect_result['data']['collectAmount']
                                print(
                                    f"{Fore.GREEN + Style.BRIGHT}\r[ Collect ] : Berhasil mengumpulkan {collected_amount} koin", flush=True)
                            else:
                                print(
                                    f"{Fore.RED + Style.BRIGHT}\r[ Collect ] : Gagal mengumpulkan koin", flush=True)
            else:
                print(
                    f"{random.choice(available_colors)+Style.BRIGHT}\r[ Recovery ] : No recovery available", flush=True)
            time.sleep(2)
            print(
                f"{random.choice(available_colors)+Style.BRIGHT}\r[ Free Chest ] : Trying to collect..", end="", flush=True)
        ...
        for detik in range(waktu_tunggu, 0, -1):
            ...
            time.sleep(1)
        sys.stdout.write(
            "\rWaktu claim berikutnya telah tiba!                                                          \n")
